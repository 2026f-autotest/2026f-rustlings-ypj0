...
unsafe fn modify_by_address(address: usize) {
    // SAFETY: The caller guarantees that `address` is a valid, aligned,
    // non-null pointer to a mutable `u32` that is not aliased by any other
    // reference for the duration of this call. We simply cast the address
    // to a `*mut u32` and write a new value through it.
    unsafe {
